    ...
    with zipfile.ZipFile(filename, 'r') as z:
        ...
        for s in sheets:
            if s['name'] == 'Movement V.030N':
                ...
                print("Sample rows from Movement V.030N:")
                for r_idx in sorted(rows.keys())[5:15]:
                    print(f"Row {r_idx}:")
                    for col in sorted(rows[r_idx].keys(), key=lambda x: (len(x), x)):
                        print(f"  {col}: {repr(rows[r_idx][col])}")
                break
